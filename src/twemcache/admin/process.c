...
#define METRIC_PRINT_FMT "%33s %15s %s\r\n"
#define METRIC_PRINT_LEN 120 /* 34 (name) + 16 (type) + 68 (description) + CRLF */
...
_admin_stats(struct response *rsp, struct request *req)
{
    ...
    for (int i = 0; i < card; ++i) {
        offset += metric_print(stats_buf + offset, stats_len - offset, &gs[i]);
    }

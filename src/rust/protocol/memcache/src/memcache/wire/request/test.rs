...
//! Tests for the `Memcache` protocol implementation.
...
fn get() {
    ...
    for keyset in keysets {
        ...
        if let MemcacheRequest::Get { keys } = request.into_inner() {
            println!("keys: {:?}", keys);
...
fn gets() {
    ...
    for keyset in keysets {
        ...
        let request = parser.parse(buffer.as_bytes()).expect("parse failure");
        assert_eq!(request.consumed(), buffer.len());
        if let MemcacheRequest::Gets { keys } = request.into_inner() {
            println!("keys: {:?}", keys);
            assert_eq!(keys.len(), keyset.len());
            for (id, key) in keyset.iter().enumerate() {
                assert_eq!(keys[id].as_ref(), key.as_bytes());
...
fn set() {
    ...
    for key in &keys {
        for value in &values {
            ...
            let request = parser.parse(buffer.as_bytes()).expect("parse failure");
            assert_eq!(request.consumed(), buffer.len());
            if let MemcacheRequest::Set { entry, noreply } = request.into_inner() {
...
fn prepend() {
    ...
    for key in &keys {
        for value in &values {
            ...
            let request = parser.parse(buffer.as_bytes()).expect("parse failure");
            assert_eq!(request.consumed(), buffer.len());
            if let MemcacheRequest::Prepend { entry, noreply } = request.into_inner() {
                assert_eq!(entry.key(), key.as_bytes());
                assert_eq!(entry.value(), Some((*value).into()));
                assert_eq!(entry.cas(), None);
                assert!(!noreply);
...
fn cas() {
    ...
    let request = parser.parse(buffer).expect("parse failure");
    assert_eq!(request.consumed(), buffer.len());
    if let MemcacheRequest::Cas { entry, noreply } = request.into_inner() {
        assert_eq!(entry.key(), b"0");
        assert_eq!(entry.value().unwrap(), Value::U64(0));
...
fn delete() {
    ...
    for k in &keys {
        ...
        let request = parser.parse(buffer.as_bytes()).expect("parse failure");
        assert_eq!(request.consumed(), buffer.len());
        if let MemcacheRequest::Delete { key, noreply } = request.into_inner() {
            assert_eq!(key.as_ref(), k.as_bytes());
            assert!(!noreply);
        } else {
            panic!("invalid parse result");
...
fn flush_all() {
    ...
    let request = parser.parse(buffer).expect("parse failure");
    assert_eq!(request.consumed(), buffer.len());
    if let MemcacheRequest::FlushAll = request.into_inner() {
    } else {
        panic!("invalid parse result");
    }
}

...
impl RRequest for MemcacheRequest {
...
impl MemcacheParser {
    ...
    fn parse_delete(buffer: &mut BytesMut) -> Result<MemcacheRequest, ParseError> {
        ...
        let consumed = if noreply {
            key_end + b"noreply".len() + CRLF_LEN
        } else {
            key_end + CRLF_LEN
        };
...
impl Request {
    pub fn process(self, config: &Arc<Config>, write_buffer: &mut BytesMut, data: &mut SegCache) {
        match self {
...
pub fn process_get(request: GetRequest, write_buffer: &mut BytesMut, data: &mut SegCache) {
    let mut found = 0;
...
pub fn process_gets(request: GetsRequest, write_buffer: &mut BytesMut, data: &mut SegCache) {
    let mut found = 0;
...
pub fn process_delete(request: DeleteRequest, write_buffer: &mut BytesMut, data: &mut SegCache) {

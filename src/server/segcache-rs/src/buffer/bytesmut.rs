...
impl Parse<MemcacheRequest> for BytesMut {
    fn parse(&mut self) -> Result<MemcacheRequest, ParseError> {
        match parse_command(self)? {
            MemcacheCommand::Get => parse_get(self),
            MemcacheCommand::Gets => parse_gets(self),
            MemcacheCommand::Set => parse_set(self),
            MemcacheCommand::Add => parse_add(self),
            MemcacheCommand::Replace => parse_replace(self),
            MemcacheCommand::Cas => parse_cas(self),
            MemcacheCommand::Delete => parse_delete(self),
        }
    }
}

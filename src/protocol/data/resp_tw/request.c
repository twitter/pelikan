...
#define REQUEST_MODULE_NAME "protocol::resp-tw::request"
...
request_borrow(void)
{
    ...
    if (req == NULL) {
        /* TODO(sean): What is the equivalent code in resp trying to do here? */
        log_debug("borrow req failed: OOM");
    ...
    log_vverb("borrowing request %p", req);
...
request_setup(request_options_st *options, request_metrics_st *metrics)
{
    ...
    if (options != NULL) {
        ...
        for (size_t i = 1; i < REQ_SENTINEL; ++i) { /* update nopt based on ntoken */
            if (command_table[i].nopt == OPT_UNLIMITED) {
...
request_teardown(void)
{
    ...
    if (!request_init) {
        log_warn("%s has never been set up", REQUEST_MODULE_NAME);
    }

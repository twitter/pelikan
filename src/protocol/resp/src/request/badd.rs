...
impl BAddRequest {
    ...
    pub fn inner_key_value_pairs(&self) -> Box<[(&[u8], &[u8])]> {
        let arc_pairs: &[ArcKeyValuePair] = &self.inner_key_value_pairs;
        let mut non_arc_pairs = Vec::<(&[u8], &[u8])>::new();

        for arc_pair in arc_pairs {
            let non_arc_pair: (&[u8], &[u8]) = (&arc_pair.0, &arc_pair.1);
            non_arc_pairs.push(non_arc_pair);
        }

        non_arc_pairs.into_boxed_slice()
    }
}

...
static void
test_assert_append_basic_entry_exists(struct bstring key)
{
    struct item *it = item_get(&key);
    ck_assert_msg(it != NULL, "item_get could not find key %.*s", key.len, key.data);
    ck_assert_msg(it->is_linked, "item with key %.*s not linked", key.len, key.data);
    ck_assert_msg(!it->in_freeq, "linked item with key %.*s in freeq", key.len, key.data);
    ck_assert_msg(!it->is_raligned, "item with key %.*s is raligned", key.len, key.data);
    ck_assert_int_eq(it->vlen, sizeof("key") - 1 + sizeof("append") - 1);
    ck_assert_int_eq(it->klen, sizeof("key") - 1);
    ck_assert_int_eq(cc_memcmp(item_data(it), "val" "append", sizeof("key") - 1 + sizeof("append") - 1), 0);
}

static void
test_assert_prepend_basic_entry_exists(struct bstring key)
{
    struct item *it = item_get(&key);
    ck_assert_msg(it != NULL, "item_get could not find key %.*s", key.len, key.data);
    ck_assert_msg(it->is_linked, "item with key %.*s not linked", key.len, key.data);
    ...
    ck_assert_int_eq(it->vlen, sizeof("key") - 1 + sizeof("prepend") - 1);
    ck_assert_int_eq(it->klen, sizeof("key") - 1);
    ck_assert_int_eq(cc_memcmp(item_data(it), "prepend" "val", sizeof("key") - 1 + sizeof("prepend") - 1), 0);
}
static void
test_assert_annex_sequence_exists(struct bstring key, uint32_t len, const char* literal)
{
    struct item *it = item_get(&key);
    ck_assert_msg(it != NULL, "item_get could not find key %.*s", key.len, key.data);
    ck_assert_msg(it->is_linked, "item with key %.*s not linked", key.len, key.data);
...
static void
test_assert_expire_basic_exists(struct item *it, struct bstring key)
{
    it = item_get(&key);
...
static void
test_assert_expire_truncated_exists(struct item *it, struct bstring key)
{
    it = item_get(&key);
    ck_assert_msg(it != NULL, "item_get on unexpired item not successful");
    proc_sec += (10 + 2);
...
#define APPEND "append"
    ...
    test_assert_append_basic_entry_exists(key);
...
#define PREPEND "prepend"
    ...
    test_assert_prepend_basic_entry_exists(key);
...
#define APPEND2 "append2"
    ...
    test_assert_annex_sequence_exists(key, val.len + append1.len, VAL APPEND1);
    ...
    test_assert_annex_sequence_exists(key, prepend.len + val.len + append1.len, PREPEND VAL APPEND1);
    ...
    test_assert_annex_sequence_exists(key,  val.len + append1.len + prepend.len + append2.len, PREPEND VAL APPEND1 APPEND2);
...
#define TIME 12345678
    ...
    test_assert_expire_basic_exists(it, key);
...
#define TTL_LONG (TTL_MAX + 5)
    ...
    test_assert_expire_truncated_exists(it, key);

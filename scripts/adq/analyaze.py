        ...
        for item_size in (64, 4096):
            for nconn in (100, 500, 1000, 2000, 5000, 10000, ):
                ...
                if len(configs) == 0:
                    ...
                    # print("{:8} {:8} {:8} {:8} {:8} {:8} {:8} {:8.4} {:8} {:8}".format(
                    #     np.mean(sorted([config.P25   for config in configs])[:]).astype(int),
                    #     np.mean(sorted([config.P50   for config in configs])[:]).astype(int),
                    #     np.mean(sorted([config.P75   for config in configs])[:]).astype(int),
                    #     np.mean(sorted([config.P90   for config in configs])[:]).astype(int),
                    #     np.mean(sorted([config.P99   for config in configs])[:]).astype(int),
                    #     np.mean(sorted([config.P999  for config in configs])[:]).astype(int),
                    #     np.mean(sorted([config.P9999 for config in configs])[:]).astype(int),
                    #     np.mean(sorted([config.thrpt for config in configs])[:]), nconn, item_size
                    # ))
                    print("{:8} {:8} {:8} {:8} {:8} {:8} {:8} {:8.4} {:8} {:8}".format(
                        np.median(sorted([config.P25   for config in configs])[:]).astype(int),
                        np.median(sorted([config.P50   for config in configs])[:]).astype(int),
                        np.median(sorted([config.P75   for config in configs])[:]).astype(int),
                        np.median(sorted([config.P90   for config in configs])[:]).astype(int),
                        np.median(sorted([config.P99   for config in configs])[:]).astype(int),
                        np.median(sorted([config.P999  for config in configs])[:]).astype(int),
                        np.median(sorted([config.P9999 for config in configs])[:]).astype(int),
                        np.median(sorted([config.thrpt for config in configs])[:]), nconn, item_size
                    ))
...
def used():
    plot_lat("rpcperf_log_mellanox_40Gbps_tuning", "rpcperf_log_mellanox_adq3", )

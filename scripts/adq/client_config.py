...
]'''.format(threads=threads, connections=conn_per_thread, window=connections//6000+3, nkey=nkey, rate=rate,
...
def generate_runscript(binary, server_ip, instances):
    ...
    with open(fname, 'w') as the_file:
        ...
        #     the_file.write('{binary_file} --config {config_file}'.format(
        #         binary_file=binary, config_file='rpcperf.toml'))
        ...
        the_file.write(' --endpoint {server_ip}:{server_port}'.format(
            server_ip=server_ip, server_port=server_port))
        the_file.write(' > rpcperf_{server_port}_{instance_idx}.log'.format(
            server_port=server_port, instance_idx=0))
        the_file.write(' 2>&1 &\n')

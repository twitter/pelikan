...
layout: post
comments: true
title:  "Segcache: a new in-memory storage design with high memory-efficiency and scalability"
...
In-memory caches are widely deployed in modern web applications to provide
...
backend. This serves as a convenient backdrop to design, test, and integrate
Segcache, an idea borne out of collaboration between [Carnegie Mellon University](http://www.cs.cmu.edu/~rvinayak/#group)
and Twitter.
...
them. They either incur high computation overhead, using a lot of CPU and memory
bandwidth, or fail to remove most expired objects in a timely fashion. The
detailed discussion can be found in Section 2.1 of [our paper](https://www.usenix.org/system/files/nsdi21-yang.pdf).
...
In Segcache, we prioritize expiration over eviction by efficiently removing
expired objects. It further offers the features to achieve high memory
...
4. A merge-based eviction algorithm to preserve objects with high access counts.
...
therefore, expiration time. This property allows Segcache to quickly indentify
expired segments simply by looking at the first segment of each TTL bucket. If
it the first segment is expired, all objects in it are removed, and the segment
...
object indexes for eviction and other cache operations. These activity can limit
...
shows that how much DRAM we should provision for each design to keep the current
...
optimization we performed to make Pelikan-segcache work well as a fully-fledged
cache backend.
